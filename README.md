...
`main` at `v0.11.0`, pushed to [DamoDCoder/signal-garden](https://github.com/DamoDCoder/signal-garden). Everything passes under `task check`.
...
**M3's metrics foundation, capacity controls, and load generator are done.** `GET /metrics` is a Prometheus scrape target: tick duration, gRPC/REST call duration, events processed, snapshots dropped, WebSocket freshness, and `pending` (consumer lag), summed across every run rather than last-writer-wins. Deliberately no `run_id` label anywhere, to keep cardinality bounded — see [0016](docs/decisions/0016-prometheus-metrics-carry-no-run-id-label.md). `worker_count` and `batch_size` are real `Controls` fields: together they cap how many records one tick folds, as a capacity model rather than literal goroutines — see [0017](docs/decisions/0017-worker-count-and-batch-size-are-a-capacity-model-not-goroutines.md). `task load` drives a running daemon over its real gRPC API with a controlled burst and reports what it observed — the feedback demo is one command now. Daemon and contract only so far; no client sliders yet. Still open: `retries`, OpenTelemetry traces, the in-app performance view, and failure injection — see [docs/roadmap.md](docs/roadmap.md).
...
Metrics, worker-count/batch-size, and the load generator are done — see above. Left: failure injection, OpenTelemetry traces, and client sliders for the two new controls. It is also where two deferrals finally get numbers instead of guesses:
