...
`main` at `v0.13.0`, pushed to [DamoDCoder/signal-garden](https://github.com/DamoDCoder/signal-garden). Everything passes under `task check`.

Nothing here blocks the client. M1 and M2's exit criteria are both met, and the contract is generated, versioned, and checked against the generator that consumes it. M3's build items are done in both repos; two exit criteria are still open — see below.

**M3's metrics foundation, capacity controls, load generator, failure injection, and traces are done.** `GET /metrics` is a Prometheus scrape target: tick duration, gRPC/REST call duration, events processed, snapshots dropped, WebSocket freshness, `pending` (consumer lag, summed across every run rather than last-writer-wins), and snapshot-save retries/failures. Deliberately no `run_id` label anywhere, to keep cardinality bounded — see [0016](docs/decisions/0016-prometheus-metrics-carry-no-run-id-label.md). `worker_count` and `batch_size` are real `Controls` fields: together they cap how many records one tick folds, as a capacity model rather than literal goroutines — see [0017](docs/decisions/0017-worker-count-and-batch-size-are-a-capacity-model-not-goroutines.md). `task load` drives a running daemon over its real gRPC API with a controlled burst and reports what it observed. `Controls.fail_snapshot_every` makes the periodic on-disk snapshot save fail and retry — deliberately not event processing, where every outcome is permanent given the garden's state, so there's nothing transient to retry; see [0018](docs/decisions/0018-failure-injection-targets-the-snapshot-save-not-event-processing.md). Run/event correlation — the thing metrics deliberately couldn't carry — is OpenTelemetry traces now: a span per gRPC call and per tick, exported OTLP/gRPC to an optional local endpoint (`SIGNAL_GARDEN_OTEL_ENDPOINT`, unset by default, costs nothing) — see [0019](docs/decisions/0019-traces-are-tick-and-rpc-grained-not-per-event.md). The client (`app.signal-garden` `v0.4.0`) has caught up too: sliders for all three new controls, both new counters in its Pressure panel, and a `compose.observability.yaml` overlay (`task observability:up`) that brings up Prometheus and Jaeger alongside the stack for exactly this. Every M3 *build* item has shipped across both repos; two exit criteria remain open — a documented bottleneck, and recovery time specifically — see [docs/roadmap.md](docs/roadmap.md).
...
Every build item is done — metrics, worker-count/batch-size, the load generator, failure injection, traces, and the client's sliders and Pressure-panel additions, see above. Two exit criteria are still open: at least one bottleneck measured and improved or explicitly documented, and recovery time specifically (repeatable failure *scenarios* already exist, from M2's crash matrix and `fail_snapshot_every`). This is also where two deferrals finally get numbers instead of guesses, and either could be the bottleneck that closes the first open item:
