...
Between buf and protoc, the deciding factor is that the annotation protos are two stable files that have not changed meaningfully in years. Vendoring them costs 13KB and removes both a binary install and a network dependency from `make proto`. buf's real advantages — breaking-change detection and lint — matter when a contract has external consumers negotiating compatibility; this one has a React client in the same repository. Adopting buf later is a Makefile change, because the `.proto` files themselves are unchanged by this decision.
